...
data <- read_delim("E:/ISEP 2/MON DOSSIER/APPRENTISSAGE DES VACANCES/ISEP3/Projet BSA/BASES DE DONNEES INITIALES/Senegal.csv",
...
filtered_data_Senegal <- filtered_data_Senegal %>% drop_na(a_partir_de_POINT)
...
missing_values <- sapply(filtered_data_Senegal, function(x) sum(is.na(x)))
...
duplicates <- filtered_data_Senegal %>%
...
# Enregistrer 
write_xlsx(filtered_data_Senegal, "E:/ISEP 2/MON DOSSIER/APPRENTISSAGE DES VACANCES/ISEP3/Projet BSA/BASES DE DONNEES CORRIGEES/Base_Senegal_Nettoyee.xlsx")
