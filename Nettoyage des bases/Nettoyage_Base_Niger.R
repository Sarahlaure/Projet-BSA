...
data <- read_delim("E:/ISEP 2/MON DOSSIER/APPRENTISSAGE DES VACANCES/ISEP3/Projet BSA/BASES DE DONNEES INITIALES/Niger.csv",
...
filtered_data_Niger <- filtered_data_Niger %>% drop_na(a_partir_de_POINT)
...
missing_values <- sapply(filtered_data_Niger, function(x) sum(is.na(x)))
...
duplicates <- filtered_data_Niger %>%
...
# Enregistrer 
write_xlsx(filtered_data_Niger, "E:/ISEP 2/MON DOSSIER/APPRENTISSAGE DES VACANCES/ISEP3/Projet BSA/BASES DE DONNEES CORRIGEES/Base_Niger_Nettoyee.xlsx")
