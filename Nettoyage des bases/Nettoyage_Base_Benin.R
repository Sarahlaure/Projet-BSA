...
data <- read_delim("E:/ISEP 2/MON DOSSIER/APPRENTISSAGE DES VACANCES/ISEP3/Projet BSA/BASES DE DONNEES INITIALES/Benin.csv",
...
filtered_data_Benin <- filtered_data_Benin %>% drop_na(a_partir_de_POINT)
...
missing_values <- sapply(filtered_data_Benin, function(x) sum(is.na(x)))
...
duplicates <- filtered_data_Benin %>%
...
# Enregistrer 
write_xlsx(filtered_data_Benin, "E:/ISEP 2/MON DOSSIER/APPRENTISSAGE DES VACANCES/ISEP3/Projet BSA/BASES DE DONNEES CORRIGEES/Base_Benin_Nettoyee.xlsx")
